...
// ── KPI Card ──────────────────────────────────────────────────────────────────
function KpiCard({
  label, value, sub, change, color = "brand", icon,
}: {
  label: string;
  value: string;
  sub?: string;
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-5 flex flex-col gap-3">
      <div className="flex items-start justify-between">
        ...
        {change !== undefined && (
          <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${change >= 0 ? "bg-green-50 text-green-600 dark:bg-green-900/20 dark:text-green-400" : "bg-red-50 text-red-600 dark:bg-red-900/20 dark:text-red-400"}`}>
...
// ── SVG icons ─────────────────────────────────────────────────────────────────
const IconRevenue  = () => <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>;
const IconPurchase = () => <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>;
const IconVat      = () => <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 14l6-6m-5.5.5h.01m4.99 5h.01M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2z" /></svg>;
const IconCredit   = () => <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" /></svg>;
...
const CHART_BASE: ApexOptions = {
  ...
  xaxis: { axisBorder: { show: false }, axisTicks: { show: false }, labels: { style: { fontSize: "11px", colors: "#9ca3af" } } },
  legend: { position: "top", fontFamily: "Outfit, sans-serif", fontSize: "12px", labels: { colors: "#6b7280" } },
};
...
export default function Analytics() {
  ...
  const recoveryRate    = totalOutputVat > 0 ? (totalInputVat / totalOutputVat) * 100 : 0;

  const totalSales      = g?.salesAndPaymentPerMonth.reduce((s, d) => s + d.sales,   0) ?? 0;
  ...
  const salesVsPurchasesOpts: ApexOptions = {
    ...
    yaxis: { labels: { style: { fontSize: "11px", colors: "#9ca3af" }, formatter: (v) => `₦${(v / 1_000_000).toFixed(0)}M` } },
    tooltip: { y: { formatter: (v) => fmt(v) } },
  };
  const salesVsPurchasesSeries = [
    { name: "Revenue (Invoiced)", data: g?.salesVsPurchases.map(d => d.salesAmount)    ?? [] },
  ...
  const cashFlowSeries = [
    { name: "Revenue Invoiced",    data: g?.salesAndPaymentPerMonth.map(d => d.sales)   ?? [] },
    { name: "Payment Collected",   data: g?.salesAndPaymentPerMonth.map(d => d.payment) ?? [] },
  ];
  ...
  const vatTrendOpts: ApexOptions = {
    ...
    markers: { size: 4 },
    xaxis: { ...CHART_BASE.xaxis, categories: monthLabels },
    yaxis: { labels: { style: { fontSize: "11px", colors: "#9ca3af" }, formatter: (v) => `₦${(v / 1_000).toFixed(0)}K` } },
    tooltip: { y: { formatter: (v) => fmt(v) } },
  };
  const vatTrendSeries = [
    { name: "Output VAT (payable)",  data: g?.vatTrendAnalysis.map(d => d.outputVAT) ?? [] },
    { name: "Input VAT (credit)",    data: g?.vatTrendAnalysis.map(d => d.inputVAT)  ?? [] },
  ];

  // 4. Net VAT payable per month — bar
  const netVatMonthly = g?.vatTrendAnalysis.map(d => d.outputVAT - d.inputVAT) ?? [];
  ...
  const netVatSeries = [{ name: "Net VAT Payable (monthly)", data: netVatMonthly }];

  // 5. VATable vs Non-VATable composition — line
  const vatCompositionOpts: ApexOptions = {
    ...CHART_BASE,
    chart: { ...CHART_BASE.chart, type: "line" },
    stroke: { width: [2.5, 2.5, 2.5, 2.5], curve: "smooth", dashArray: [0, 5, 0, 5] },
  ...
  return (
    <>
      <PageMeta title="Analytics | Aegis NRS Portal" description="Financial analytics and VAT intelligence" />

      {/* Header */}
      <div className="mb-6">
        <h1 className="text-xl font-semibold text-gray-800 dark:text-white">Financial Analytics</h1>
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
          12-month rolling view · {periodStart}{periodEnd ? ` – ${periodEnd}` : ""}
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
            ...
            >
              <Chart options={cashFlowOpts} series={cashFlowSeries} type="area" height={260} />
            </ChartCard>
          </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
            <div className="xl:col-span-2">
              <ChartCard
                ...
                sub="Output VAT is your FIRS liability; Input VAT is your claimable credit on purchases"
              >
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-5 flex flex-col justify-between">
              <div>
                <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">VAT Net Position</h3>
                <p className="text-xs text-gray-400 dark:text-gray-500 mb-5">12-month cumulative</p>
                <div className="space-y-3">
                  <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-gray-700">
                    <span className="text-sm text-gray-600 dark:text-gray-400">Output VAT (Sales)</span>
                    <span className="text-sm font-semibold text-amber-600 dark:text-amber-400">{fmt(totalOutputVat)}</span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-gray-700">
                    <span className="text-sm text-gray-600 dark:text-gray-400">Input Tax Credit (Purchases)</span>
                    <span className="text-sm font-semibold text-purple-600 dark:text-purple-400">({fmt(totalInputVat)})</span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-gray-700">
                    <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">Net VAT Payable to FIRS</span>
                    <span className="text-sm font-bold text-red-600 dark:text-red-400">{fmt(netVatPayable)}</span>
                  </div>
                  <div className="flex justify-between items-center py-1">
                    <span className="text-xs text-gray-500 dark:text-gray-400">VAT Recovery Rate</span>
                    <span className="text-xs font-semibold text-gray-700 dark:text-gray-300">{recoveryRate.toFixed(1)}%</span>
                  </div>
                  <div className="flex justify-between items-center py-1">
                    <span className="text-xs text-gray-500 dark:text-gray-400">VATable Revenue</span>
                    <span className="text-xs font-semibold text-gray-700 dark:text-gray-300">
                      {fmt(totalVatable)} ({pct(totalVatable, totalVatable + totalNonVatable)})
                    </span>
                  </div>
                  <div className="flex justify-between items-center py-1">
                    <span className="text-xs text-gray-500 dark:text-gray-400">Exempt / Zero-Rated</span>
                    <span className="text-xs font-semibold text-gray-700 dark:text-gray-300">
                      {fmt(totalNonVatable)} ({pct(totalNonVatable, totalVatable + totalNonVatable)})
                    </span>
                  </div>
                </div>
              </div>
              <div className="mt-5 pt-4 border-t border-gray-100 dark:border-gray-700 bg-amber-50 dark:bg-amber-900/10 rounded-xl p-3">
                <p className="text-xs text-amber-700 dark:text-amber-400 font-medium">FIRS Remittance Reminder</p>
                <p className="text-xs text-amber-600 dark:text-amber-500 mt-1">
                  VAT returns are due by the 21st of the month following the taxable period. Ensure output VAT net of input credits is remitted promptly to avoid penalties.
                </p>
              </div>
            </div>
          </div>
          ...
          <ChartCard
            ...
            sub="Output VAT minus Input Tax Credit — the amount owed to FIRS each month"
          >
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
            <ChartCard
              ...
              sub="Revenue and purchase split between VATable and zero-rated/exempt — key for FIRS compliance filing"
            >
